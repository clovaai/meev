...
class AGORA(BaseDataset):
    def __init__(self, cfg, transform, data_split):
        mainlogger.warning ("Loading AGORA dataset ...")
        ...
        mainlogger.warning ("[AGORA] " + str(self.__len__()) + " annotations")
    ...
    def load_data(self):
        ...
        if self.data_split == 'train' or (self.data_split == 'test' and self.test_set == 'val'):
            ...
            for aid in db.anns.keys():
                ...
                if self.resolution == (720, 1280):
                    ...
                    if self.cfg.parts == 'body':
                        ...
                        bbox = bbox.reshape(4)
                        bbox = process_bbox(self.cfg, bbox, img_shape[1], img_shape[0])
                        if bbox is None:
                            continue
                    ...
                    a = img['file_name_3840x2160'].split('/')[-2]
                    b = img['file_name_3840x2160'].split('/')[-1][:-4]
                    img_path = osp.join(self.data_path, '3840x2160', a + '_crop', b + '_ann_id_' + str(aid) + '.png')
                    
                    json_path = osp.join(self.annot_path, '3840x2160', a + '_crop', b + '_ann_id_' + str(aid) + '.json')
                    crop_resize_info = load_json(json_path)
                    if crop_resize_info is None:
                        continue
                    ...
                    if self.cfg.parts == 'body':
                        ...
                        data_dict = {'img_path': img_path, 'gender': gender, 'img_shape': img_shape, 'bbox': bbox, 'img2bb_trans_from_orig': img2bb_trans_from_orig, 'joints_2d_path': joints_2d_path, 'joints_3d_path': joints_3d_path, 'verts_path': verts_path, 'param_path': param_path}
            ...
            assert self.cfg.parts == 'body' # the evaluation server does not support MANO- and FLAME-only evaluations

            with open(osp.join(self.annot_path, 'AGORA_test_bbox.json')) as f:
                bboxs = json.load(f)
            ...
            for filename in bboxs.keys():
                if self.resolution == (720, 1280):
                    ...
                    for pid in range(person_num):
                        ...
                        json_path = osp.join(self.annot_path, '3840x2160', 'test_crop', filename[:-4] + '_pid_' + str(pid) + '.json')
    ...
    def __getitem__(self, idx):
        ...
        if self.cfg.parts == 'hand':
            img, img2bb_trans, bb2img_trans, rot, do_flip = augmentation(self.cfg, img, bbox, self.data_split, enforce_flip=(data['hand_type'] == 'left')) # enforce flip when left hand to make it right hand
        else:
        ...
        if self.data_split == 'train':
            ...
            #    #param = pickle.load(f, encoding='latin1')
                param = CPU_Unpickler(f, encoding='latin1').load()
            ...
            if self.cfg.parts == 'body':
                ...
                joint_img, joint_cam, joint_valid, joint_trunc = process_db_coord(self.cfg, joint_img, joint_cam, joint_valid, do_flip, img_shape, self.joint_set['body']['flip_pairs'], img2bb_trans, rot, self.joint_set['body']['joints_name'], smpl.joints_name)

                # smpl parameters
                root_pose = np.array(param['root_pose'].detach().numpy(), dtype=np.float32).reshape(-1) # rotation to world coordinate
                body_pose = np.array(param['body_pose'].detach().numpy(), dtype=np.float32).reshape(-1)
                smpl_pose = np.concatenate((root_pose, body_pose))
                shape = np.array(param['betas'].detach().numpy(), dtype=np.float32).reshape(-1)[:10] # bug?
                trans = np.array(param['translation'].detach().numpy(), dtype=np.float32).reshape(-1) # translation to world coordinate
                smpl_param = {'pose': smpl_pose, 'shape': shape, 'trans': trans}
                cam_param = {'focal': self.cfg.focal, 'princpt': self.cfg.princpt} # put random camera paraemter as we do not use coordinates from smpl parameters
                _, _, _, smpl_pose, smpl_shape, smpl_mesh_cam_orig = process_human_model_output(self.cfg, smpl_param, cam_param, do_flip, img_shape, img2bb_trans, rot, 'smpl')
                ...
                meta_info = {'mesh' : smpl_mesh_cam_orig, 'joint_valid': joint_valid, 'joint_trunc': joint_trunc, 'smpl_joint_valid': np.zeros_like(joint_valid), 'smpl_joint_trunc': np.zeros_like(joint_trunc), 'smpl_pose_valid': smpl_pose_valid, 'smpl_shape_valid': float(True), 'is_3D': float(True) }
            ...
            # hand part
                ...
                joint_img = np.concatenate((joint_img[:,:2], joint_cam[:,2:]),1) # x, y, depth
                joint_valid = np.ones_like(joint_img[:,:1])
                joint_img, joint_cam, joint_valid, joint_trunc = process_db_coord(self.cfg, joint_img, joint_cam, joint_valid, do_flip, img_shape, self.joint_set['hand']['flip_pairs'], img2bb_trans, rot, self.joint_set['hand']['joints_name'], mano.joints_name)
                ...
                cam_param = {'focal': self.cfg.focal, 'princpt': self.cfg.princpt} # put random camera paraemter as we do not use coordinates from smplx parameters
                _, _, _, mano_pose, mano_shape, _ = process_human_model_output(self.cfg, mano_param, cam_param, do_flip, img_shape, img2bb_trans, rot, 'mano')
            ...
            # face part
                ...
                joint_img = np.concatenate((joint_img[:,:2], joint_cam[:,2:]),1) # x, y, depth
                joint_valid = np.ones_like(joint_img[:,:1])
                joint_img, joint_cam, joint_valid, joint_trunc = process_db_coord(self.cfg, joint_img, joint_cam, joint_valid, do_flip, img_shape, flame.flip_pairs, img2bb_trans, rot, flame.joints_name, flame.joints_name)
                ...
                _, _, _, flame_root_pose, flame_jaw_pose, flame_shape, flame_expr, _, _ = process_human_model_output(self.cfg, flame_param, cam_param, do_flip, img_shape, img2bb_trans, rot, 'flame')
        ...
        for n in range(sample_num):
            ...
            if save_offical:
                ...
                if 'person_idx' in annot:
                    ...
                    exist_result_path = glob(osp.join(self.cfg.result_dir, 'AGORA', save_name + '*'))
                ...
                joint_proj[:,0] = joint_proj[:,0] / self.cfg.output_hm_shape[2] * self.cfg.input_img_shape[1]
                joint_proj[:,1] = joint_proj[:,1] / self.cfg.output_hm_shape[1] * self.cfg.input_img_shape[0]
                ...
                with open(osp.join(self.cfg.result_dir, 'AGORA', save_name), 'wb') as f:
    ...
    def print_eval_result(self, eval_result):
        ...
        print('AGORA test results are dumped at: ' + osp.join(self.cfg.result_dir, 'AGORA'))
