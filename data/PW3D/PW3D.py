...
class PW3D(BaseDataset):
    def __init__(self, cfg, transform, data_split):
        ...
        self.joint_set_pw3d['body']['root_joint_idx'] = self.joint_set_pw3d['body']['joints_name'].index('Pelvis')

        self.datalist = self.load_data()
        print ("[PW3D] " + str(self.__len__()) + " annotations")
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            bbox = process_bbox(self.cfg, np.array(ann['bbox']), img['width'], img['height'])
    ...
    def __getitem__(self, idx):
        ...
        # smpl coordinates
        smpl_joint_img, smpl_joint_cam, smpl_joint_trunc, smpl_pose, smpl_shape, smpl_mesh_cam_orig = process_human_model_output(self.cfg, smpl_param, cam_param, do_flip, img_shape, img2bb_trans, rot, 'smpl')
    ...
    def evaluate(self, outs, cur_sample_idx, viz=False):
        ...
        for n in range(sample_num):
            ...
            # pw3d joint from gt mesh
            mesh_gt_cam = out['smpl_mesh_cam_target']
            pose_coord_gt_pw3d = np.dot(self.joint_set_pw3d['body']['smpl_regressor'], mesh_gt_cam)
            pose_coord_gt_pw3d = pose_coord_gt_pw3d - pose_coord_gt_pw3d[self.joint_set_pw3d['body']['root_joint_idx'],None] # root-relative
            pose_coord_gt_pw3d = pose_coord_gt_pw3d[self.joint_set_pw3d['body']['eval_joint'],:]
            mesh_gt_cam -= np.dot(self.joint_set_pw3d['body']['smpl_regressor'], mesh_gt_cam)[0,None,:]
            
            # pw3d joint from output mesh
            mesh_out_cam = out['smpl_mesh_cam']
            pose_coord_out_pw3d = np.dot(self.joint_set_pw3d['body']['smpl_regressor'], mesh_out_cam)
            pose_coord_out_pw3d = pose_coord_out_pw3d - pose_coord_out_pw3d[self.joint_set_pw3d['body']['root_joint_idx'],None] # root-relative
            pose_coord_out_pw3d = pose_coord_out_pw3d[self.joint_set_pw3d['body']['eval_joint'],:]
            ...
            mesh_out_cam -= np.dot(self.joint_set_pw3d['body']['smpl_regressor'], mesh_out_cam)[0,None,:]
