...
from utils.human_models import smpl, smplx
...
class MPI_INF_3DHP(BaseDataset):
    def __init__(self, cfg, transform, data_split, visualize_info=False):
        mainlogger.warning ("Loading MPI INF 3DHP dataset ...")
        ...
        mainlogger.warning ("[MPI_INF_3DHP] " + str(self.__len__()) + " annotations")
    ...
    def __getitem__(self, idx):
        ...
        joint_img, joint_cam, joint_valid, joint_trunc = process_db_coord(self.cfg, joint_img, joint_cam, data['joint_valid'], do_flip, img_shape, self.joint_set['body']['flip_pairs'], img2bb_trans, rot, self.joint_set['body']['joints_name'], smpl.joints_name)
        ...
        meta_info = {'mesh': smpl_mesh_cam_orig, 'joint_valid': joint_valid, 'joint_trunc': joint_trunc, 'smpl_joint_trunc': smpl_joint_trunc, 'smpl_joint_valid': smpl_joint_valid, 'smpl_pose_valid': smpl_pose_valid, 'smpl_shape_valid': smpl_shape_valid, 'is_3D': float(True)}
        if self.visualize_info:
            debug = {"smpl_mesh_cam_orig": smpl_mesh_cam_orig, 'img_path': img_path, 'cam_param': cam_param}
