...
def vis_keypoints_with_skeleton(img, kps, kps_lines, kp_thresh=0.4, alpha=1, radius=3):
...
def vis_keypoints(img, kps, alpha=1, radius=3, kps_valid = None, kps_lines = [], add_text=False, font_scale= 0.2):
    # Convert from plt 0-1 RGBA colors to 0-255 BGR colors for opencv.
    cmap = plt.get_cmap('rainbow')
    colors = [cmap(i) for i in np.linspace(0, 1, len(kps) + 2)]
    colors = [(c[2] * 255, c[1] * 255, c[0] * 255) for c in colors]

    colors_lines = [cmap(i) for i in np.linspace(0, 1, len(kps_lines) + 2)]
    colors_lines = [(c[2] * 255, c[1] * 255, c[0] * 255) for c in colors_lines]
    ...
    for i in range(len(kps)):
        ...
        if add_text:
            kp_mask = cv2.putText(kp_mask, str(i), p, cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255,0,0), 1, cv2.LINE_AA)
    ...
    for l in range(len(kps_lines)):
        ...
        if kps_valid is not None:
            valid = kps_valid[i1][0].astype(np.bool_) and kps_valid[i2][0].astype(np.bool_)
        if valid:
            cv2.line(kp_mask, p1, p2, color=colors_lines[l], thickness=2, lineType=cv2.LINE_AA)
