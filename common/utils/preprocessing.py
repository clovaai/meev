...
import random
import math
from utils.augmentation import augmentation_method
...
def process_bbox(cfg, bbox, img_width, img_height, do_sanitize=True):
    if do_sanitize:
        # sanitize bboxes
        x, y, w, h = bbox
        x1 = np.max((0, x))
...
def get_aug_config(cfg):
    ...
    return scale, rot, color_scale, do_flip, dx, dy

def augmentation(cfg, img, bbox, data_split, enforce_flip=None, aug_config=None):
    if aug_config is None:
        if data_split == 'train' and getIntFromCfg(cfg, 'augmentation', 0) != -1:
            scale, rot, color_scale, do_flip, dx, dy = get_aug_config(cfg)
        else:
            scale, rot, color_scale, do_flip, dx, dy = 1.0, 0.0, np.array([1,1,1]), False, 0.0, 0.0
    else:
        scale, rot, color_scale, do_flip, dx, dy = aug_config
    ...
    if enforce_flip is None:
        ...
        do_flip = False
    img, trans, inv_trans = generate_patch_image(cfg, img, bbox, scale, rot, do_flip, cfg.input_img_shape, dx, dy)
    m = augmentation_method(cfg, data_split)
    if m is not None:
        img = m(img)
    #img = np.clip(img * color_scale[None,None,:], 0, 255)
    return img, trans, inv_trans, rot, do_flip

def generate_patch_image(cfg, cvimg, bbox, scale, rot, do_flip, out_shape, dx, dy):
    img = cvimg.copy()
    ...
    dx = 0.5
    dy = 0.5
    bb_c_x = float(bbox[0] + dx*bbox[2])
    bb_c_y = float(bbox[1] + dy*bbox[3])
...
def process_db_coord(cfg, joint_img, joint_cam, joint_valid, do_flip, img_shape, flip_pairs, img2bb_trans, rot, src_joints_name, target_joints_name):
...
def process_human_model_output(cfg, human_model_param, cam_param, do_flip, img_shape, img2bb_trans, rot, human_model_type):
